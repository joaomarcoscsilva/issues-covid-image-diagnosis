...
def get_network_fns(net, optim, batch_size = 128, parallel = True, shape = (10, 256, 256, 3)):
    ...
    def predict(params, state, X, return_representation = False, return_gradcam = False, training = True, verbose = True):
        ...
        assert not (return_representation and training)
        assert not (return_gradcam and training)
        ...
        datagen, num_batches = get_datagen(parallel, batch_size, X, include_last = True)
    ...
    def evaluate(params, state, X, Y, training = True, verbose = True):
        ...
        # Gets a data generator for the dataset
        datagen, num_batches = get_datagen(parallel, batch_size, X, Y, include_last = True)
    ...
    def train_epoch(params, state, optim_state, x_train, y_train, x_test = None, y_test = None, verbose = True):
        ...
        # Gets a data generator for the dataset
        datagen, num_batches = get_datagen(parallel, batch_size, x_train, y_train, include_last = False)
